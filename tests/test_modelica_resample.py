...
class TestModelicaResampleMath(unittest.TestCase):
    ...
    def test_simple_aggregation_single_building(self):
        ...
        pump_power = [50.0] * n_hours      # 50 W constant
        ...
        with patch('urbanopt_des.modelica_results.Reader') as mock_reader_class:
            ...
            try:
                ...
                self.assertEqual(df["Total Cooling Plant"].iloc[0], expected_cooling_plant, 
                               "Total cooling plant aggregation incorrect")
                ...
                self.assertEqual(df["Total Heating Electricity Plant"].iloc[0], 25.0,
                               "Total heating electricity plant aggregation incorrect")
                
                # Total Heating Natural Gas Plant = Boilers
                self.assertEqual(df["Total Heating Natural Gas Plant"].iloc[0], 200.0,
                               "Total heating natural gas plant aggregation incorrect")
                
                # ETS Pump Electricity Total = ambient pump + CHW pump + HHW pump
                expected_ets_pump = 8.0 + 6.0 + 7.0
                self.assertEqual(df["ETS Pump Electricity Total"].iloc[0], expected_ets_pump,
                               "ETS pump electricity total aggregation incorrect")
                
                # ETS Heat Pump Electricity Total
                self.assertEqual(df["ETS Heat Pump Electricity Total"].iloc[0], 30.0,
                               "ETS heat pump electricity total aggregation incorrect")
                ...
                self.assertAlmostEqual(df["Total DES Electricity"].iloc[0], expected_des_elec, places=2,
                                      msg="Total DES electricity aggregation incorrect")
                
                # Total DES Natural Gas = Total Heating Natural Gas Plant
                self.assertEqual(df["Total DES Natural Gas"].iloc[0], 200.0,
                               "Total DES natural gas aggregation incorrect")
                
                # Thermal energy aggregations
                self.assertEqual(df["Total Thermal Cooling Energy"].iloc[0], -500.0,
                               "Total thermal cooling energy aggregation incorrect")
                self.assertEqual(df["Total Thermal Heating Energy"].iloc[0], 600.0,
                               "Total thermal heating energy aggregation incorrect")
    ...
    def test_multiple_buildings_aggregation(self):
        ...
        with patch('urbanopt_des.modelica_results.Reader') as mock_reader_class:
            ...
            try:
                ...
                self.assertEqual(df["ETS Heat Pump Electricity Total"].iloc[0], expected_total_hp,
                ...
                self.assertEqual(df["ETS Pump Electricity Total"].iloc[0], expected_total_pump,
                ...
                self.assertEqual(df["Total Thermal Cooling Energy"].iloc[0], expected_cooling,
                               "Total thermal cooling should sum both buildings")
                
                # Total Thermal Heating = sum of both buildings
                expected_heating = 400.0 + 450.0
                self.assertEqual(df["Total Thermal Heating Energy"].iloc[0], expected_heating,
                               "Total thermal heating should sum both buildings")
    ...
    def test_fallback_to_bui_pattern(self):
        ...
        with patch('urbanopt_des.modelica_results.Reader') as mock_reader_class:
            ...
            try:
                ...
                self.assertEqual(df["ETS Pump CHW Electricity Building bldg1"].iloc[0], 2.5,
                               "Should retrieve CHW pump from bui pattern")
                self.assertEqual(df["ETS Pump HHW Electricity Building bldg1"].iloc[0], 3.5,
                               "Should retrieve HHW pump from bui pattern")
