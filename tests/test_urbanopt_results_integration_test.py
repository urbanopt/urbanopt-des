...
class UrbanoptResultsIntegrationTest(unittest.TestCase):
    ...
    # Artifacts that the URBANopt post-process step (`save_dataframes` +
    # `create_aggregations`) regenerates inside ``run/<scenario>/output``.
    # Only these are removed by setUp — other files written into the same
    # directory by downstream notebooks (e.g. ``building_metrics_annual.csv``)
    # are preserved.
    UO_GENERATED_PATTERNS = ("loads_*.csv", "power_*.csv")

    # Files regenerated by the Modelica post-process step
    # (``ModelicaResults.save_variables`` and the per-scenario summaries).
    MODELICA_GENERATED_PATTERNS = (
        "modelica_variables.json",
        "*_15min.csv",
        "*_60min.csv",
        "*_annual.csv",
        "*_monthly.csv",
    ...
    def setUp(self):
        ...
        # Surgical cleanup of the URBANopt scenario output directory: only
        # remove the CSVs that ``save_dataframes`` will rewrite. This keeps
        # any other artifacts (e.g. ``building_metrics_annual.csv``) intact.
        uo_output_dir = self.data_dir / "three_building_test" / "run" / "baseline" / "output"
        if uo_output_dir.is_dir():
            for pattern in self.UO_GENERATED_PATTERNS:
                for path in uo_output_dir.glob(pattern):
                    if path.is_file():
                        path.unlink()

        # Same idea for the Modelica scenarios: don't nuke the whole output
        # directory tree, just the files the test will regenerate.
        modelica_root = self.data_dir / "three_building_test_des_agg"
        for output_dir in modelica_root.rglob("output"):
            if not output_dir.is_dir():
                continue
            for pattern in self.MODELICA_GENERATED_PATTERNS:
                for path in output_dir.glob(pattern):
                    if path.is_file():
                        path.unlink()

        # Modelica variables JSON files live alongside the .mat files (not
        # under output/), so they need their own pass.
        for path in modelica_root.rglob("modelica_variables.json"):
            if path.is_file():
                path.unlink()
